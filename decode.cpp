...
ALU_Ops decode(const ALU_Type data) {
  if (data.instr.lsli.op == ALU_LSLI_OP) {
    ...
    if (opts.instrs) {
      cout << "lsls r" << data.instr.lsli.rd << ", r" << data.instr.lsli.rm
           << ", #" << data.instr.lsli.imm
           << endl;  // not entirely sure about this one??
    }
    return ALU_LSLI;
  } else if (data.instr.addr.op == ALU_ADDR_OP) {
    if (opts.instrs) {
      cout << "adds r" << data.instr.addr.rd << ", r" << data.instr.addr.rn
           << ", r" << data.instr.addr.rm << endl;
    }
    return ALU_ADDR;
  } else if (data.instr.subr.op == ALU_SUBR_OP) {
    // 315: insert code here to print subs instruction
    if (opts.instrs) {
      cout << "subs r" << data.instr.subr.rd << ", r" << data.instr.subr.rn
           << ", r" << data.instr.subr.rm << endl;
    }
    return ALU_SUBR;
  } else if (data.instr.add3i.op == ALU_ADD3I_OP) {
    // complete
    if (opts.instrs) {
      cout << "adds r" << data.instr.add3i.rd << ", r" << data.instr.add3i.rn
    ...
    // 315: insert code here to print subs instruction
    if (opts.instrs) {
      cout << "subs r" << data.instr.sub3i.rd << ", r" << data.instr.sub3i.rn
    ...
    // complete
    if (opts.instrs) {
      cout << "adds r" << data.instr.add8i.rdn << ", #" << setbase(10)
           << data.instr.add8i.imm << endl;
    }
    return ALU_ADD8I;
  } else if (data.instr.sub8i.op == ALU_SUB8I_OP) {
    // 315: insert code here to print subs instruction
    if (opts.instrs) {
      cout << "subs r" << data.instr.sub8i.rdn << ", #" << setbase(10)
           << data.instr.sub8i.imm << endl;
    }
    return ALU_SUB8I;
  } else if (data.instr.cmp.op == ALU_CMP_OP) {
    // complete
    if (opts.instrs) {
      cout << "cmp r" << data.instr.cmp.rdn << ", #" << setbase(10)
           << data.instr.cmp.imm << endl;
    }
    return ALU_CMP;
  } else if (data.instr.mov.op == ALU_MOV_OP) {
    // complete
    if (opts.instrs) {
      cout << "movs r" << data.instr.mov.rdn << ", #" << setbase(10)
           << (data.instr.mov.imm) << endl;
    }
    return ALU_MOV;
  }
}
...
DP_Ops decode(const DP_Type data) {
  if (data.instr.DP_Instr.op == DP_CMP) {
    ...
    if (opts.instrs) {
      cout << "cmp r" << data.instr.DP_Instr.rdn << ", r"
           << data.instr.DP_Instr.rm << endl;
    }
    return DP_CMP;
  } else {
...
SP_Ops decode(const SP_Type data) {
  if (data.instr.mov.op == 2) {
    if (opts.instrs) {
      ...
      if (data.instr.mov.d) {
        ...
        if (data.instr.mov.rd == 5) {
          ...
          cout << " r" << setbase(10) << (8 + data.instr.mov.rd) << ", sp"
               << endl;
        }
        // this case is for registers greater than r7 that aren't sp
        else {
          cout << " r" << setbase(10) << (8 + data.instr.mov.rd) << ", r"
               << setbase(10) << data.instr.mov.rm << endl;
        }
      }
      // another stack pointer case
        ...
        cout << " r" << setbase(10) << data.instr.mov.rd << ", r"
             << data.instr.mov.rm << endl;
      }
    }
    return SP_MOV;
    ...
    if (opts.instrs) {
      cout << "add sp, sp, r" << setbase(10) << data.instr.add.rm
           << endl;  // possibly wrong??
    }
    return SP_ADD;
    ...
    if (opts.instrs) {
      cout << "cmp sp, sp"
           << endl;  // unsure about this one leave like this for now
    }
    return SP_CMP;
  } else {
...
LD_ST_Ops decode(const LD_ST_Type data) {
  if (data.instr.class_type.opA == LD_ST_REG_OPA) {
    if (data.instr.class_type.opB == LD_ST_OPB_LDRB) {
      ...
      if (opts.instrs) {
        cout << "ldrb r" << data.instr.ld_st_reg.rt << ", [r"
             << data.instr.ld_st_reg.rn << ", r" << data.instr.ld_st_reg.rm
             << "]" << endl;
      }
      return LDRBR;
      ...
      if (opts.instrs) {
        cout << "ldr r" << data.instr.ld_st_reg.rt << ", [r"
             << data.instr.ld_st_reg.rn << ", r" << data.instr.ld_st_reg.rm
             << "]" << endl;
      }
      return LDRR;
    ...
    if (data.instr.class_type.opB == LD_ST_OPB_STR) {
      if (opts.instrs) {
        cout << "str r" << data.instr.ld_st_imm.rt << ", [r"
             << data.instr.ld_st_imm.rn << ", #" << setbase(10)
             << (data.instr.ld_st_imm.imm * 4) << "]" << endl;
      }
      return STRI;
    } else if (data.instr.class_type.opB == LD_ST_OPB_LDR) {
      if (opts.instrs) {
        cout << "ldr r" << data.instr.ld_st_imm.rt << ", [r"
             << data.instr.ld_st_imm.rn << ", #" << setbase(10)
             << (data.instr.ld_st_imm.imm * 4) << "]" << endl;
      }
      return LDRI;
    ...
    if (data.instr.ld_st_imm.op == LD_ST_LDB) {
      // 315: write code to print ldrb
      if (opts.instrs) {
        cout << "ldrb r" << data.instr.ld_st_imm.rt << ", [r"
             << data.instr.ld_st_imm.rn << ", #" << setbase(10)
             << (data.instr.ld_st_imm.imm * 4) << "]" << endl;
      }
      return LDRBI;
      ...
      // 315: write code to print strb
      if (opts.instrs) {
        cout << "strb r" << data.instr.ld_st_imm.rt << ", [r"
             << data.instr.ld_st_imm.rn << ", #" << setbase(10)
             << (data.instr.ld_st_imm.imm * 4) << "]" << endl;
      }
      return STRBI;
    ...
    if (data.instr.ld_st_sp_imm.opB) {
      if (opts.instrs) {
        cout << "ldr r" << data.instr.ld_st_sp_imm.rt << ", [sp, #"
             << setbase(10) << data.instr.ld_st_sp_imm.imm << "]" << endl;
        return LDRSP;
      }
    } else {
      if (opts.instrs) {
        cout << "str r" << data.instr.ld_st_sp_imm.rt << ", [sp, #"
             << setbase(10) << data.instr.ld_st_sp_imm.imm << "]" << endl;
        return STRSP;
      }
    }
...
MISC_Ops decode(const MISC_Type data) {
  if (data.instr.push.op == MISC_PUSH_OP) {
    ...
    if (opts.instrs) {
      cout << "sub sp, #";
      cout << setbase(10)
           << (static_cast<unsigned int>(data.instr.sub.imm) << 2) << endl;
    }
    return MISC_SUB;
    ...
    if (opts.instrs) {
      cout << "add sp, #";
      cout << setbase(10)
           << (static_cast<unsigned int>(data.instr.add.imm) << 2) << endl;
    }
    return MISC_ADD;
...
int decode(const COND_Type data) {
  ...
  if (opts.instrs) {
    ...
    cout << " 0x" << hex << rf[15] + 2 * (int)((char)(data.instr.b.imm)) + 2
         << endl;
  }
}
...
int decode(const UNCOND_Type data) {
  // complete
  if (opts.instrs) {
    cout << "b 0x" << hex << rf[15] + 2 * (int)((char)(data.instr.b.imm)) + 2
         << endl;
  }
}
...
int decode(const LDM_Type data) {
  ...
  if (opts.instrs) {
    cout << "ldm r" << data.instr.ldm.rn << "!, {r" << data.instr.ldm.reg_list
         << "}" << endl;  // unsure if needs ldm.reg_list part??
  }
  return LDM;
}
...
int decode(const STM_Type data) {
  ...
  if (opts.instrs) {
    cout << "stm r" << data.instr.stm.rn << "!, {r" << data.instr.stm.reg_list
         << "}" << endl;
  }
  return STM;
}
